"""causal_conv1d with a fused per-head FWHT butterfly rotation (LinQuant).

Fork of `causal_conv1d.py`: identical conv math, plus a register-level Hadamard applied to
the conv output before it is stored. Same channel spec as the CUDA kernel
(`linquant/kernels/conv_butterfly.py`): `signs` float32[dim], `log2dim` int8[dim], 0 = identity.
Rotating in BOTH the prefill and the decode path keeps the recurrent state in one basis.

Diff vs the vendor file: `_fwht_stage`/`_rotate` below, two extra kernel params
(`signs_ptr`, `log2dim_ptr`) with `ROTATE`/`DMAX` constexprs, one `_rotate` call per epilogue.
"""
import numpy as np
import torch
...
@triton.jit()
def _fwht_stage(x, d, N: tl.constexpr, NG: tl.constexpr, DMAX: tl.constexpr, H: tl.constexpr,
                UNIFORM: tl.constexpr):
    """One FWHT butterfly stage at distance H. With mixed head dims a channel whose head is
    already fully mixed (H >= its d) passes through; UNIFORM compiles that select away."""
    if H < DMAX:
        v = tl.reshape(x, (NG, DMAX // (2 * H), 2, H))
        sg = tl.where(tl.arange(0, 2)[None, None, :, None] == 0, 1.0, -1.0)
        b = v * sg + tl.flip(v, 2)
        if UNIFORM:
            v = b
        else:
            v = tl.where(tl.reshape(d, (NG, DMAX // (2 * H), 2, H)) > H, b, v)
        x = tl.reshape(v, (N,))
    return x


@triton.jit()
def _rot_spec(idx_feats, mask_feats, signs_ptr, log2dim_ptr, ROT_D: tl.constexpr):
    """Per-channel (head dim, signs/sqrt(d)). Token-invariant -- hoist out of the token loop."""
    if ROT_D > 0:
        d = tl.full(idx_feats.shape, ROT_D, tl.int32)
    else:
        d = (1 << tl.load(log2dim_ptr + idx_feats, mask=mask_feats, other=0).to(tl.int32))
    scale = tl.load(signs_ptr + idx_feats, mask=mask_feats, other=0.0)
    return d, scale * tl.rsqrt(d.to(tl.float32))
...
def _rotate(acc, d, scale, mask_feats, BLOCK_N: tl.constexpr, DMAX: tl.constexpr,
    ...
    x = _fwht_stage(x, d, BLOCK_N, NG, DMAX, 8, UNIFORM)
...
def _causal_conv1d_fwd_kernel(  # continuous batching
    ...
    if ROTATE:
        rot_d, rot_scale = _rot_spec(idx_feats, idx_feats < dim,
                                     signs_ptr, log2dim_ptr, ROT_D)
    ...
    for idx_token in range(segment_len):
        ...
        if ROTATE:
            acc = _rotate(acc, rot_d, rot_scale, idx_feats < dim,
                          BLOCK_N, DMAX, ROT_D)
...
def causal_conv1d_fn(
    ...
    metadata=None,
    validate_data=False,
    rot_signs: torch.Tensor | None = None,
    rot_log2dim: torch.Tensor | None = None,
    rot_d: int = 0,
):
    ...
    _causal_conv1d_fwd_kernel[grid](
        ...
        BLOCK_N=256,
        signs_ptr=rot_signs,
        log2dim_ptr=rot_log2dim,
        ROTATE=rot_signs is not None,
        DMAX=rot_d if rot_d else 128,
        ROT_D=rot_d,
        num_stages=2,
    )
